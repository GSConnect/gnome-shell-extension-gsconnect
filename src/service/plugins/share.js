...
/**
 * Get the main application window to use as a transient parent.
 *
 * @returns {Gtk.Window|null} The parent window, or %null
 */
function _getShareParent() {
    const parent = Gio.Application.get_default().get_active_window();

    if (parent === null)
        return null;

    const typeName = GObject.type_name(parent.constructor.$gtype);

    if (typeName.startsWith('GSConnect') && typeName.endsWith('Window'))
        return parent;
...
const SharePlugin = GObject.registerClass({
     ...
     */
    share() {
        const dialog = new ShareDialog(this, _getShareParent());
        dialog.present();
    }
...
/** A simple GTK4 dialog for sharing files or links */
const ShareDialog = GObject.registerClass({
    GTypeName: 'GSConnectShareDialog',
    Template: 'resource:///org/gnome/Shell/Extensions/GSConnect/ui/share-dialog.ui',
    Children: [
        'title-widget', 'open-check', 'files-button', 'uri-entry',
        'link-button',
    ],
}, class ShareDialog extends Adw.ApplicationWindow {

    _init(plugin, parent = null) {
        super._init({
        ...
        const fileLabel = _('Send files to %s').format(this.plugin.device.name);
        this.files_button.label = fileLabel;
        this.files_button.tooltip_text = fileLabel;

        // TRANSLATORS: eg. Send a link to Google Pixel
        this.link_button.tooltip_text = _('Send a link to %s').format(
            this.plugin.device.name
        );
    }
    ...
    _onChooseFiles() {
        const fileDialog = new Gtk.FileDialog({
            // TRANSLATORS: eg. Send files to Google Pixel
            title: _('Send files to %s').format(this.plugin.device.name),
        });

        fileDialog.open_multiple(this, null, (dialog, result) => {
            try {
                const files = dialog.open_multiple_finish(result);

                for (let i = 0, len = files.get_n_items(); i < len; i++) {
                    const file = files.get_item(i);
                    this.plugin.shareFile(file.get_uri(), this.open_check.active);
                }
    ...
    _onSendLink() {
        const uri = this.uri_entry.text.trim();

        if (!uri)
            return;

        this.plugin.shareUri(uri);
        this.close();
    }
});
