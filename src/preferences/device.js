...
export const DeviceNavigationPage = GObject.registerClass({
    ...
    */
     ...
     * 
     * @param {GLib.Settings} settings - The settings object where the plugin configurations are stored.
     * @param {string|null} key - The key that was changed in the settings. It could be either 'disabled-plugins' 
     *                            or 'supported-plugins'. If no specific key is passed, it triggers updates for all plugins.
     * @returns {void}
     */
    _addPlugin(name) {
    ...
    /**
     * Handles changes to the receive directory setting.
     * 
     * This method is triggered when the receive directory setting changes. It checks if the new directory is valid; 
     * if not, it assigns a default directory (the user's Downloads folder). If the directory is updated, it updates 
     * the subtitle of the related UI element to reflect the new path.
     * 
     * @param {Gio.Settings} settings - The settings object containing the receive directory key.
     * @param {string} key - The key corresponding to the receive directory setting.
     * @returns {void}
     */
     ...
     * 
     * @param {Gtk.Widget} widget - The widget (usually a button) that triggered the action.
     */
    _onReceiveDirectoryChanged(settings, key) {
        let receiveDir = settings.get_string(key);
